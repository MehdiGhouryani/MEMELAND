...
def leaderboard_text(period="week", user_id=None):
    # ⚠️ UTC — signals.created_at حالا سراسر UTC ذخیره می‌شه (نگاه کن به
    # db/signals_repo.py). این تابع همون get_leaderboard_rows رو صدا می‌زنه
    # که /site/leaderboard هم صدا می‌زنه؛ اگه اینجا محلی می‌موند، لیدربورد
    # داخل چت با لیدربورد وب‌اپ برای همون بازه‌ی زمانی عدد متفاوت می‌داد.
    if period == "week":
