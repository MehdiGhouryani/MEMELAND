/**
 * MemeLand Trader Dossier Module
 */

const Dossier = {
  async show(userId) {
    if (!userId) {
      if (window.TGBridge) TGBridge.showAlert('شناسه کاربری ثبت نشده است');
      return;
    }
    ...
    body.innerHTML = '<div style="text-align:center; padding:30px; font-size:12px; color:var(--text-muted);">در حال دریافت اطلاعات کالر...</div>';
    sheet.classList.add('show');
    if (window.TGBridge) TGBridge.syncBackButton(true);

    try {
      const resp = await fetch(`/site/traders/${userId}`, { headers: API.getHeaders() });
      ...
      body.innerHTML = `
        <div style="display:flex; align-items:center; gap:14px; margin-bottom:16px;">
          <div class="avatar-frame theme-${roleKey}" style="width:60px; height:60px; min-width:60px; min-height:60px;">
            <div style="width:100%; height:100%;">${avatarSvg}</div>
            <span class="role-badge-mini">${miniBadge}</span>
          </div>
          <div>
            <h3 style="font-size:15px; margin-bottom:4px;">${data.display_name}</h3>
          ...
          ${
            ...
            : data.recent_signals.map(s => `
                <div style="display:flex; justify-content:space-between; align-items:center; background:var(--bg); padding:7px 10px; border-radius:8px; font-size:11px;">
                  <span>${s.coin} <span style="color:var(--text-muted); font-size:9px;">(${s.channel ? s.channel.toUpperCase() : 'DEX'})</span></span>
      ...
      body.innerHTML = '<div style="text-align:center; padding:30px; font-size:12px; color:var(--red);">پرونده این معامله‌گر یافت نشد یا ثبت نشده است.</div>';
    }
  }
};